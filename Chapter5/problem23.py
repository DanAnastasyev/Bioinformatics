...
for i in range(1, n):
    for j in range(1, m):
        best_move = max([(res[i-1, j] + indel_penalty, BackTrack.down),
...
def print_alignment(backtrack, i, j, first_line, second_line, alignment):
    while i >= 0 and j >= 0:
        if backtrack[i, j] == BackTrack.diag:
            alignment[0] += first_line[-1]
            alignment[1] += second_line[-1]
            i -= 1
            j -= 1
...
alignment = ['', '']
print_alignment(backtrack, n-1, m-1, first_line, second_line, alignment)
with open('output.txt', 'w', encoding='utf8') as f:
    print(res[n-1, m-1], file=f)
